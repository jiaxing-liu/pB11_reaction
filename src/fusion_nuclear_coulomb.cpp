...
#include "fusion_fsci_coulomb_tables.inc"
#include <cmath>
namespace {
...
static int nuclear_impl(int channel,double input,fusion_nuclear_coulomb_v1 *out,
 const Segments& segments,double common_radius) {
    ...
    *out={};
    ...
    for(const auto &segment:segments) {
        ...
        const double mu=3727.3794118*(channel<3?2./3:.5),a=common_radius>0?common_radius:(channel<3?5.1:4.5);
...
extern "C" int fusion_c_nuclear_coulomb(int channel,double input,
 ...
 return nuclear_impl(channel,input,out,fusion_table_data::segments,0);
}
extern "C" int fusion_c_nuclear_coulomb_radius16(int channel,double input,
 fusion_nuclear_coulomb_v1* out){
 return nuclear_impl(channel,input,out,fusion_fsci_table_data::segments,16);
}
